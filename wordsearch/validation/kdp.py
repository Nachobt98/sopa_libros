...
class KdpPreflightIssue:
    ...
    def format(self) -> str:
        ...
        return f"[{prefix}] {self.message}: {self.path}" if self.path else f"[{prefix}] {self.message}"
...
class KdpPreflightReport:
    ...
    def print_summary(self) -> None:
        print("\n=== KDP preflight ===")
        print(f"Formato: {self.format_name}")
...
def build_kdp_preflight_report(
    *,
    ...
    if pdf_exists:
        _inspect_pdf(pdf_path, report, layout=layout)
...
def write_kdp_preflight_report(report: KdpPreflightReport, *, output_dir: str) -> str:
    ...
    Path(report_path).write_text(json.dumps(report.to_dict(), ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    return report_path
...
def _inspect_pdf(pdf_path: str, report: KdpPreflightReport, *, layout: LayoutConfig) -> None:
    path = Path(pdf_path)
    try:
    ...
    if report.actual_page_count != report.expected_page_count:
        report.add_error(f"El PDF tiene {report.actual_page_count} paginas reales, pero se esperaban {report.expected_page_count}", path=str(path))
    if reader.pages:
        ...
        _check_pdf_page_size(first_width, first_height, path=str(path), report=report, layout=layout)
        for index, page in enumerate(reader.pages[1:], start=2):
            ...
            if not _same_size(width, first_width) or not _same_size(height, first_height):
                report.add_error(f"La pagina {index} no tiene el mismo tamano que la primera ({width}x{height} in frente a {first_width}x{first_height} in)", path=str(path))
...
def _page_size_in(page: Any) -> tuple[float, float]:
    box = page.mediabox
    return round(float(box.width) / PDF_POINTS_PER_INCH, 4), round(float(box.height) / PDF_POINTS_PER_INCH, 4)


def _check_pdf_page_size(width_in: float, height_in: float, *, path: str, report: KdpPreflightReport, layout: LayoutConfig) -> None:
    if not _same_size(width_in, layout.trim_width_in) or not _same_size(height_in, layout.trim_height_in):
        report.add_error(f"El tamano fisico del PDF no coincide con el trim esperado {layout.trim_width_in}x{layout.trim_height_in} in; recibido {width_in}x{height_in} in", path=path)
...
def _normalize_pdf_metadata(metadata: Any) -> dict[str, str | None]:
    ...
    key_map = {"/Title": "title", "/Author": "author", "/Subject": "subject", "/Keywords": "keywords", "/Creator": "creator", "/Producer": "producer"}
...
def _check_pdf_metadata(report: KdpPreflightReport) -> None:
    ...
    for key, expected_value in report.expected_pdf_metadata.items():
        ...
        if actual_value != expected_value:
            report.add_warning(f"Metadata PDF inesperada para '{key}': esperado '{expected_value}', recibido '{actual_value}'", path=report.pdf_path)


def _check_page_images(image_paths: list[str], report: KdpPreflightReport, *, label: str) -> None:
    if not image_paths:
    ...
    for image_path in image_paths:
        ...
        try:
            with Image.open(path) as image:
                if image.size != (report.expected_page_width_px, report.expected_page_height_px):
                    report.add_error(f"La imagen {label} no coincide con el tamano esperado {report.expected_page_width_px}x{report.expected_page_height_px}px; recibido {image.size[0]}x{image.size[1]}px", path=str(path))
